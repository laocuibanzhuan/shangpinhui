<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item, index) in address" :key="item.id">
        <span class="username" :class="{ selected: item.isDefault == 1 }">
          {{ item.consignee }}</span
        >
        <p @click="selectAddress(item)">
          <span class="s1"
            >{{ item.userAddress }}&nbsp;&nbsp;&nbsp;{{ item.fullAddress }}</span
          >
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>{{ tradeInfo.originalTotalAmount }}</span>
      </div>
      <div
        class="receiveInfo"
    ...
    <div class="sub clearFix">
      <a class="subBtn" @click="commit">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    ...mapState("trade", ["address"]),
    selectedAddress() {
      let selected = false;
      this.address.forEach((ele) => {
        if (ele.isDefault == 1) {
          selected = ele;
        }
      });
  ...
  methods: {
    ...
    async commit() {
      let res = await this.$store.dispatch("trade/commitOrder", {
        traderNo: this.tradeInfo.tradeNo,
        ...
        paymentWay: "ONLINE",
        orderComment: this.message || "nothing",
