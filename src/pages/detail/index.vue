<template>
  <div class="detail">
    ...
    <section class="con">
      <div class="conPoin">
        <span v-for="(item, index) in detailInfo.categoryView" :key="index" v-if="item">{{
          item
        }}</span>
      </div>
      <div class="mainCon">
        <div class="previewWrap">
          <Zoom v-if="skuImageList[0]" :skuImageList="skuImageList" />
          <ImageList v-if="skuImageList[0]" :skuImageList="skuImageList" />
        </div>
            ...
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  ...
                  :key="item_.id"
                  :class="{ active: item_.isChecked == '1' }"
                  @click="selectAttr(item_, item.spuSaleAttrValueList)"
                >
                  {{ item_.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  @change="change"
                />
                <a class="plus" @click="jia">+</a>
                <a class="mins" @click="jian">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  computed: {
    ...mapState("detail", ["detailInfo"]),
    ...mapGetters("detail", ["skuInfo", "spuSaleAttrList", "skuImageList"]),
  },
  methods: {
    selectAttr(select, otherList) {
      otherList.forEach((item) => {
        item.isChecked = "0";
      });
      select.isChecked = "1";
      this.attr[select.saleAttrName] = select.saleAttrValueName;
    },
    defaultSelectAttr() {
      for (let i = 0; i < this.spuSaleAttrList.length; i++) {
        for (let j = 0; j < this.spuSaleAttrList[i].spuSaleAttrValueList.length; j++) {
          if (this.spuSaleAttrList[i].spuSaleAttrValueList[j].isChecked == 1) {
            this.attr[this.spuSaleAttrList[i].saleAttrName] = this.spuSaleAttrList[
              i
            ].spuSaleAttrValueList[j].saleAttrValueName;
          }
        }
      }
    },
    ...
    async addToCart() {
      let info = [this.num, parseInt(this.$route.params.goodsId)];
  ...
  mounted() {
    this.$store.dispatch("detail/getDetail", this.$route.params.goodsId);
  },
  updated() {
    if (!this.initAttr) {
